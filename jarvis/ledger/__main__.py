"""Command-line auditor for a Glass Ledger.
    ...
    python -m jarvis.ledger verify LEDGER --pubkey HEX [--pin FILE] [--json]
    python -m jarvis.ledger LEDGER --pubkey HEX --pin FILE      (same thing)
    python -m jarvis.ledger tail LEDGER [-n 10]
    python -m jarvis.ledger keygen DIR
    python -m jarvis.ledger pubkey KEYFILE
    python -m jarvis.ledger repair LEDGER
...
from jarvis.ledger.chain import (generate_key, load_private_key, public_key_hex,
                                   repair_torn_tail, parse_line, scan_tail)
from jarvis.ledger.verify import verify_file, load_pin, save_pin
...
def cmd_verify(args) -> int:
    ...
    if args.json:
        ...
        if report.torn_tail:
            print(f"repair: python -m jarvis.ledger repair {args.ledger}   "
...
def main(argv=None) -> int:
    ...
    ap = argparse.ArgumentParser(prog="python -m jarvis.ledger", description=__doc__,
