# Jarvis agent-first AMI
# ========================
# Builds an Amazon Machine Image whose primary process is the Jarvis
...
# 2023 AMI, uploads build/jarvis-src.tar.gz (git archive of HEAD), runs
# aws/scripts/provision.sh and aws/scripts/cleanup.sh, then snapshots it
# as "jarvis-agent-<base>-<arch>-<timestamp>".
...
locals {
  ...
  tags = merge({
    ...
    Project         = "jarvis"
    Role            = "agent-first-os"
    JarvisVersion = var.jarvis_version
    JarvisProfile = "cloud"
    BaseImage       = var.base
...
source "amazon-ebs" "jarvis" {
  region          = var.region
  instance_type   = var.instance_type
  ami_name        = local.ami_name
  ami_description = "Jarvis agent-first OS ${var.jarvis_version} (${var.base}, ${var.arch})"
...
build {
  name    = "jarvis-ami"
  sources = ["source.amazon-ebs.jarvis"]

  # Upload the repository as a tarball made by `make ami-bundle`
  # (git archive of HEAD, so the image matches a commit exactly).
  provisioner "file" {
    source      = "build/jarvis-src.tar.gz"
    destination = "/tmp/jarvis-src.tar.gz"
  }

  provisioner "shell" {
    execute_command = "chmod +x {{ .Path }}; sudo env SRC=/tmp/jarvis-src JARVIS_VERSION=${var.jarvis_version} {{ .Path }}"
