...
operator repairs that explicitly with ``python -m openclaw.ledger repair``.
...
class TornTail(LedgerError):
    ...
    def __init__(self, path: str, offset: int):
        super().__init__(f"torn tail in {path}: partial final line after byte {offset}; "
                         f"repair with: python -m openclaw.ledger repair {path}")
        self.path = path
        self.offset = offset
...
class LedgerWriter:
    ...
    def __init__(self, path: str, private_key, writer: str = "openclaw",
                 lock: bool = True):
        self.path = path
